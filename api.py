...
def configure_ngrok():
    ngrok_token = "2luIQiSU1mp67vWpoayet9LuLJN_2AavLP9rhw6P1TojB6py8"
    try:
...
def update_soul_txt(public_url):
    with open("xnxx.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in xnxx.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("xnxx.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated xnxx.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update xnxx.txt on VPS: {str(e)}")
